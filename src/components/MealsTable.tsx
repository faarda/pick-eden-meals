      ...
      <tbody>
        {
          meals.map((meal, index) => {
            ...
            return <tr>
              <td className={`px-3 lg:px-5 py-5 border-gray-200 bg-white text-xs md:text-pre-xs lg:text-sm ${isLast ? 'border-0' : 'border-b'}`}>
                ...
                >
                  <svg width="12" height="8" viewBox="0 0 22 15" fill="none" xmlns="http://www.w3.org/2000/svg" className={`transition-opacity duration-150 ${isInList(index) ? 'opacity-1' : 'opacity-0'}`}>
                    <path d="M2 7.54264L8.6 13L20 2" stroke="white" stroke-width="3" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </label>
              </td>
              ...
              {
                meal.map(food => {
                  return <td className={`min-w-30 px-3 lg:px-5 py-5  border-gray-200 bg-white text-xs md:text-pre-xs lg:text-sm text-gray-600 text-left max-w-75 ${isLast ? 'border-0' : 'border-b'}`}>
                    { food }
                  </td>
                })
              }
